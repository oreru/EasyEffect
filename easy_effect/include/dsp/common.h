...
#if defined(__FAST_MATH__) && (__apple_build_version__ >= 16000000) && \
    (__apple_build_version__ <= 16000099)
...
Major versions are not compatible with each other.  Minor and patch versions are
...
constexpr bool versionCheck(int major, int minor, int patch = 0) {
  return major == SIGNALSMITH_DSP_VERSION_MAJOR &&
         (SIGNALSMITH_DSP_VERSION_MINOR > minor ||
          (SIGNALSMITH_DSP_VERSION_MINOR == minor &&
           SIGNALSMITH_DSP_VERSION_PATCH >= patch));
}

/// Check the library version is compatible (semver).
#define SIGNALSMITH_DSP_VERSION_CHECK(major, minor, patch) \
  static_assert(                                           \
      ::signalsmith::versionCheck(major, minor, patch),    \
      "signalsmith library version is " SIGNALSMITH_DSP_VERSION_STRING);

/** @} */
}  // namespace signalsmith
#else
// If we've already included it, check it's the same version
static_assert(SIGNALSMITH_DSP_VERSION_MAJOR == 1 &&
                  SIGNALSMITH_DSP_VERSION_MINOR == 7 &&
                  SIGNALSMITH_DSP_VERSION_PATCH == 0,
              "multiple versions of the Signalsmith DSP library");
#endif  // include guard
